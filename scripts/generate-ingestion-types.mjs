// S2-003 TypeScript contract type generator.
// Reads the nine canonical ingestion schemas from contracts/ and emits
// src/lib/ingestion/contracts.d.ts. The schemas are the single source of
// truth: this file must never be edited by hand — regenerate with
//   node scripts/generate-ingestion-types.mjs --write
// tests/ingestion/contracts.types.test.mjs fails closed if the committed
...
export const INGESTION_CONTRACTS = Object.freeze([
  'source-descriptor',
  'connector-contract',
  'fetch-request',
  'source-snapshot',
...
function main() {
  ...
  if (current !== output) {
    console.error('DRIFT: src/lib/ingestion/contracts.d.ts does not match contracts/*.schema.json');
    console.error('regenerate with: node scripts/generate-ingestion-types.mjs --write');
    process.exit(1);
  }
  console.log(JSON.stringify({ ok: true, bytes: output.length, contracts: INGESTION_CONTRACTS.length }));
}
