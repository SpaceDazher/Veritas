// S2-005 DB-backed independent replay.
...
// OWN child process (distinct PID, executor id, nonce and clock — todo §9
// requires process separation) against a PostgreSQL-backed claim graph store.
// The coordinator never runs corpus logic itself.
//
//   node scripts/s2-005-db-replay.mjs            # compare, write results/
//   node scripts/s2-005-db-replay.mjs --write    # bind evidence/ artifacts
//
...
import { runCorpus } from './s2-005-run.mjs';
import { PostgresClaimGraphStore } from '../src/lib/claims/postgres-store.mjs';
import { makeAuthorityRegistry } from '../src/lib/claims/store.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const CONTAINER = 'veritas-s2-005-db-replay';
...
async function coordinator(args) {
  ...
  try {
    ...
    const outDir = writeEvidence ? 'evidence' : 'results/s2-005';
    fs.mkdirSync(path.join(ROOT, outDir), { recursive: true });

    const runParams = [
      { schema: 'run_a', runId: 's2-005-db-run-a', executorId: 'exec-db-a', nonce: 'n-dbsynthreplay01', clock: '2026-01-15T08:00:00.000Z' },
      { schema: 'run_b', runId: 's2-005-db-run-b', executorId: 'exec-db-b', nonce: 'n-dbsynthreplay02', clock: '2026-03-21T23:59:59.999Z' },
    ];
    const summaries = [];
    for (const [i, params] of runParams.entries()) {
      const outFile = path.join(ROOT, outDir, `s2-005-db-run-${i === 0 ? 'a' : 'b'}.json`);
      const childEnv = { ...process.env, VERITAS_DB_URL: connectionString };
      const child = spawnSync(process.execPath, [
        path.join(ROOT, 'scripts/s2-005-db-replay.mjs'),
    ...
    const report = {
      schemaVersion: 1,
      role: 'S2-005 DB-backed run A vs run B comparison (PostgreSQL claim graph stores, separate OS processes)',
    ...
    const outPath = path.join(ROOT, outDir, 's2-005-db-comparison.json');
    fs.writeFileSync(outPath, `${JSON.stringify(report, null, 2)}\n`);
    console.log(JSON.stringify({ ok: report.comparison.ok, issues, digest: a.graphDigest, cases: a.caseCount }, null, 2));
